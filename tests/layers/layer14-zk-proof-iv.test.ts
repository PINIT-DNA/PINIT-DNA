...
jest.mock('../../src/lib/prisma', () => ({
  prisma: { zkProofLayer: { upsert: jest.fn(async () => ({})) } },
}));
...
function proofDataOf(call: unknown): string {
  return (call as { create: { proofData: string } }).create.proofData;
}
...
describe('Layer 14 — proof data encryption', () => {
  test('the IV differs across two calls for the SAME owner (the actual bug)', async () => {
    ...
    const [iv1] = proofDataOf(zkUpsert.mock.calls[0]![0]).split(':');
    const [iv2] = proofDataOf(zkUpsert.mock.calls[1]![0]).split(':');
  ...
  test('proofData is iv:ciphertext:authTag and actually decrypts with that IV', async () => {
    ...
    const proofData = proofDataOf(zkUpsert.mock.calls[0]![0]);
