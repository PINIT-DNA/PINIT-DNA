...
function DnaDetailModal({ record, onClose }: { record: DnaRecord; onClose: () => void }) {
  const { user } = useAuth();
  return (
    <Modal open title="Protected file details" onClose={onClose} size="lg">
      <div className="p-6 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: 'Record ID',       value: record.id,                mono: true,  accent: true },
            ...
            { label: 'Type',            value: record.imageMimeType,     mono: false, accent: false },
            { label: 'File Size',       value: formatBytes(record.imageSizeBytes), mono: false, accent: false },
            { label: 'Category',        value: deriveFileType(record), mono: false, accent: false, typeBadge: true },
            { label: 'Created At',      value: format(new Date(record.createdAt), 'PPpp'), mono: false, accent: false },
            { label: 'Owner',           value: user?.shortId ?? '—', mono: true, accent: true },
            { label: 'Digital Asset',   value: record.vaultId ?? 'Not stored yet', mono: true, accent: !!record.vaultId },
          ].map(row => (
  ...
  return (
    <div className="page-shell space-y-5 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-white">Protected assets</h1>
              ...
              {loading ? (
                ...
                <tr><td colSpan={7}>
                  <EmptyState
                    ...
                    description="Protect an asset to see records here"
                  />
                </td></tr>
              ) : (
                filtered.map(r => (
