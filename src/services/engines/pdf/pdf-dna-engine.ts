...
import { simHash64, computeLayer6IdentitySeal, sha256, shannonEntropy } from '../base/text-utils';
...
export class PdfDnaEngine {
  ...
  private layer6(fingerprints: string, dnaRecordId: string): UniversalLayerResult {
    const t = Date.now();
    const hmac = computeLayer6IdentitySeal('PDF', fingerprints, dnaRecordId, config.stego.signatureSecret);
    return {
